...
export function FloatingChatButton() {
  ...
  function submitQuestion(question: string) {
    ...
    window.setTimeout(() => {
      ...
      setMessages((items) => [
        ...items,
        { id: Date.now() + 1, role: "assistant", text: buildAnswer(trimmed) }
      ]);
    }, 520);
  }
...
function buildAnswer(question: string) {
  ...
  if (text.includes("report") || text.includes("summary") || text.includes("plant")) {
    ...
    return `Plant summary: ${machines.length} machines monitored. ${critical.length} critical, ${warning.length} warning, and ${machines.length - critical.length - warning.length} good. Highest priority is ${critical[0]?.name || "none"} because of vibration and low RUL. Recommended action: inspect bearings, reduce load, and keep the next maintenance slot active.`;
  }

  if (text.includes("alert")) {
    return priorityAlerts
      .map((a) => `${a.severity}: ${a.machine} - ${a.description} (${a.time})`)
      .join("\n");
  }

  if (text.includes("maintenance") || text.includes("schedule") || text.includes("next")) {
    const next = maintenanceSchedule[0];
    return `Next maintenance: ${next.machine}, ${next.type}, ${next.date} ${next.window}. Assigned engineer: ${next.engineer}. I recommend keeping this slot because current predictions show the shortest RUL on the Pressure Screen PS-800.`;
  }

  if (text.includes("rul") || text.includes("failure") || text.includes("prediction")) {
    const prediction = machine
      ? failurePredictions.find((p) => p.machine === machine.name)
    ...
    return `${prediction.machine} has predicted failure code ${prediction.code}, estimated time ${prediction.eta}, and ${prediction.confidence}% confidence. Suggested action: schedule inspection before the ETA and verify sensor readings.`;
  }

  if (text.includes("critical") || text.includes("why") || text.includes("issue") || text.includes("screen") || text.includes("digester") || text.includes("dryer") || text.includes("calender") || text.includes("hydra") || text.includes("paper")) {
    ...
    return `${target.name} is at ${target.health}/100 health with ${target.rul} RUL. Main reason: ${cause}. Recommended action: reduce load, inspect rotating components, validate sensor readings, and assign ${target.engineer} for follow-up.`;
  }

  if (text.includes("energy") || text.includes("loss")) {
    return "Production loss is 8.2%, down 15% versus the last 7 days. The biggest avoidable energy risk is running machines in warning or critical state; prioritize Pressure Screen PS-800 and Dryer Cylinders DC-600 to reduce load-related waste.";
  }

  return "I can help with machine health, current alerts, RUL predictions, maintenance schedules, failure codes, and plant summaries. Try asking: Why is Pressure Screen PS-800 critical?";
}

function findMachine(text: string) {
  return machines.find((m) =>
    text.includes(m.name.toLowerCase()) ||
    text.includes(m.area.toLowerCase()) ||
    m.name.toLowerCase().split(" ").some((word) => word.length > 4 && text.includes(word.toLowerCase()))
  ) || null;
}
